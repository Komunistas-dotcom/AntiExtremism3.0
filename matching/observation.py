...
TRACE_TYPES: dict[str, str] = {
    ...
    "like": "лайк",
    "repost": "переслано вами",
    # Полученное отделено от отправленного намеренно. Сообщение из
    # экстремистского канала, пересланное вам собеседником, тоже хранится
    # в вашей переписке и потому проверяется — но это принципиально другое
    # действие, чем пересылка своими руками, и в отчёте это должно быть видно.
    "repost_received": "переслано вам (получено)",
    "view": "история просмотров",
    "link": "ссылка, отправленная вами",
    "link_received": "ссылка, полученная в переписке",
...
# Следы, которые пользователь не создавал сам, а получил.
RECEIVED_TRACE_TYPES = frozenset({"repost_received", "link_received"})
